...
// FIXME: this is an error, no more replies can be received
            ...
            // TODO: convert a readable into a stream.Readable, and deal only with event emitters
    ...
    function call( handlerName, data, callback ) {
        ...
        var envelope = { v: 1, id: id, n: handlerName, m: undefined, b: undefined }
        Buffer.isBuffer(data) ? envelope.b = data : envelope.m = data
    ...
    function wrap( object, methods, options ) {
        ...
        for (var i=0; i<methods.length; i++) {
            ...
            caller[method] = (function(method) { return function() {
                var av = new Array(arguments.length)
                for (var i=0; i<arguments.length; i++) av[i] = arguments[i]
                if (av[0]) av[0] = self.message._extractError(av[0])
                var cb = (av.length > 0 && typeof av[av.length-1] === 'function') ? av.pop() : null
                self.call(prefix + method, av, function(err, args) {
                    if (cb) err ? invoke(cb, [err]) : invoke(cb, args)
                })
            }})(method)
        }
        return caller

        function hashToStruct( obj ) {
            ...
            function f () {}
            f.prototype = obj
            try { return obj } catch (err) { }
        }
    },

    _deliverResponses:
    function _deliverResponses( doneDelivering ) {
        // TODO: join() converts buffers to strings, which is not binary safe
        var start = 0, end = 0
