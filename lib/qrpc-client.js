...
QrpcClient.prototype = {
    ...
    function call( handlerName, data, callback ) {
        ...
        if (!handlerName) return callback(new Error("no handler specified"))
        if (callback) this.callbacks[id] = callback
else console.log("AR: no callback passed in!!")
...
console.log("AR: abortAllCalls:", err, err.stack)
        for (var i in this.callbacks) {
// FIXME: running 40k calls in series clears the callbacks ?!
// (setImmediate() every 40th else stack overflows)
// Works ok if every call is done with a setImmediate()
            var cb = this.callbacks[i]
            if (cb) cb(err)
            else console.log("internal error: id '" + i + "' is not a callback", cb)
        }
        this.callbacks = {}
    },
