...
QrpcServer.prototype = {
    ...
    function createServer( options, connectListener ) {
        ...
        server.once('error', function(err) {
            return self._throwError(err, callback)
        })
    ...
    _dispatchCalls:
    function _dispatchCalls( calls, socket ) {
        var self = this
        var i, msg
        for (i in calls) {
            msg = calls[i]
            ...
            var res = new QrpcResponse(msg.id, socket)
            msg.body = msg.m
            if (handler) handler(msg, res, function(/*VARARGS*/) {
                var retv = new Array(arguments.length)
                if (arguments.length > 0) for (var i=0; i<arguments.length; i++) retv[i] = arguments[i]
                // TODO: delay response if socket buffering is full
                res.end({ v: 1, id: id, m: retv })
            })
        }
    },
    ...
    function _decodeCalls( data, calls ) {
        ...
        while ((end = data.indexOf('\n', start)) >= 0) {
            ...
            if (call instanceof Error) {
                ...
                self._logError(msg, "error: unable to decode call: " + line)
            }
            else calls.push(call)
            start = end + 1
        }
        return start < data.end ?  data.slice(start) : ""
    },

    // decode a newline terminated call spec (json bundle)
    _decodeCall:
    function _decodeCall( line ) {
        if (line.slice(0, 7) === '{"v":1,') {
    ...
    _sendResponse:
    function _sendResponse( version, socket, response ) {
        if (version === 1) {
