...
QrpcServer.prototype = {
    ...
    function onData( oldData, chunk, writeStream ) {
        var data = (typeof oldData === 'object')
            ? (oldData ? Buffer.concat([oldData, chunk]) : chunk)
            : (oldData ? oldData + chunk : "" + chunk)
        var calls = new Array()
    ...
    // add handlers for the named methods of the object (default all)
    wrap:
    function wrap( object, methods, options ) {
        ...
        for (var i=0; i<methods.length; i++) {
            if (typeof object[i] === 'function') wrapMethod(prefix + i, object, object[i])
        }

        var self = this
        function wrapMethod( name, object, method ) {
            self.addHandler(name, function(req, res, next) {
    ...
    function _decodeCalls( data, calls ) {
        ...
        while ((end = (isString ? data.indexOf('\n', start) : this._indexOf(data, '\n', start))) >= 0) {
        ...
        return start < data.length ?  data.slice(start) : null
    },
