...
QrpcServer.prototype = {
    ...
    function listen( port, callback ) {
        ...
        if (!this.server) return this._throwError(new Error("call setNetServer first"), callback)
        if (this._listening) return this._throwError(new Error("already listening on port " + this._port), callback)
        var self = this
        this.server.listen(port, function() {
            self._listening = true
            self._port = port
            if (callback) callback(null, this.socket)
        })
    },
